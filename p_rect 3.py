...
def fragment_collision_check_func(rect_vx,rect_vy,rect_omega,rect_datax,rect_datay,rect_y,rect_rotaxis_x,rect_rotaxis_y):
    ...
    if distance_y<=0:
        if ground_distance>0:
            ...
            rect_rotaxis_x=collision_point_x
            if distance_x<=0: #если прямоугольники пересекаются по х, то они сталкиваются
                rect_vy=((rect_y+rect_h/2)*(rect_m+rect2_m)/(rect_m+rect2_m)+2*rect2_m*rect2_vy0/(rect_m+rect2_m))*rect_k
                if distance_x<rect_w: #если прямоугольник выходит за границы, то он отталкивается в сторону
                    rect_vx=-5
                    rect_omega=np.pi/3.5
        else:
            collision_point_y=rect_y_min
            collision_point_x=rect_datax[np.argmin(rect_datay)] #np.argmin возвращает индекс нижней точки, с помощью которого можно получить координату по х
            rect_rotaxis_y=collision_point_y
            rect_rotaxis_x=collision_point_x
            rect_vy=((rect_y+rect_h/2)*(rect_m+ground_m)/(rect_m+ground_m)+2*ground_m*ground_vy0/(rect_m+ground_m))*rect_k
