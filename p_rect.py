...
def fragment_collision_check_func(rect_vx,rect_vy,rect_omega,rect_datax,rect_datay,rect_a,rect_y):
    distance_x=min(rect2_datax0)-max(rect_datax)
    distance_y=min(rect_datay)-max(rect2_datay0)
    if distance_y<=0:
        if distance_x<=0:
        if rect_a<=np.pi/4:
            rect_vy=rect_y*(rect_m+rect2_m)/(rect_m+rect2_m)+2*rect2_m*rect2_vy0/(rect_m+rect2_m)
            rect_vx=-5
            ...
            rect_omega=0
    return rect_vx,rect_vy,rect_omega

def seq_calc_func(rect_x,rect_y,rect_vx,rect_vy,rect_a,rect_omega):
    ...
    for frame in range(frames-1):
        ...
        fragment_rect_datax=rect_fig_x*np.cos(rect_a)-rect_fig_y*np.sin(rect_a)+rect_x
        fragment_rect_datay=rect_fig_y*np.cos(rect_a)+rect_fig_x*np.sin(rect_a)+rect_y

        seq_rect_datax.append(fragment_rect_datax)
        seq_rect_datay.append(fragment_rect_datay)

        rect_vx,rect_vy,rect_omega=fragment_collision_check_func(rect_vx,rect_vy,rect_omega,fragment_rect_datax,fragment_rect_datay,rect_a,rect_y)
...
if __name__=='__main__':
    ...
    seq_rect_datax,seq_rect_datay=seq_calc_func(rect_x0,rect_y0,rect_vx0,rect_vy0,rect_a0,rect_omega0)
