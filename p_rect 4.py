...
def fragment_collision_check_func(rect_vx,rect_vy,rect_omega,rect_datax,rect_datay,rect_rotaxis_x,rect_rotaxis_y):
        ...
        if ground_distance>0:
            if distance_x<=0: #если прямоугольники пересекаются по х, то они сталкиваются
                ...
                if distance_x<rect_w: #если прямоугольник выходит за границы, то он отталкивается в сторону
                    rect_rotaxis_x,rect_rotaxis_y=get_collision_point_func(rect_datax,rect_datay)
                    rect_vx=-5
...
def get_collision_point_func(rect_datax,rect_datay):
    collision_point_index=np.argmin(rect_datay) #np.argmin возвращает индекс нижней точки по y
    collision_point_x=rect_datax[collision_point_index]
    collision_point_y=rect_datay[collision_point_index]
...
def seq_calc_func(rect_x,rect_y,rect_vx,rect_vy,rect_a,rect_omega,rect_rotaxis_x,rect_rotaxis_y):
    ...
    for frame in range(frames-1):
        ...
        print(rect_rotaxis_x,rect_rotaxis_y)

        seq_rect_datax.append(fragment_rect_datax)
        seq_rect_datay.append(fragment_rect_datay)

        rect_vx,rect_vy,rect_omega,rect_rotaxis_x,rect_rotaxis_y=fragment_collision_check_func(rect_vx,rect_vy,rect_omega,fragment_rect_datax,fragment_rect_datay,rect_rotaxis_x,rect_rotaxis_y)
